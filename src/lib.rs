...
impl Runtime {
    ...
    pub fn push(&mut self, mut task: Task) -> Result<usize> {
        let mut available = self.uring.submission().available();

        if available.is_full() {
            anyhow::bail!("uring is full")
        }
        ...
        let entry = entry.user_data(task_id as _);
    ...
    pub fn run(&mut self) -> Result<(usize, Task, Result<i32>)> {
